			...
			for (int i = 0; i < dealStatusArray.length(); i++) {
				...
				if (indDealLob.get("lobValue").equals(
					...
					for (int j = 0; j < dealStagesArray.length(); j++) {
						...
						if(kcontext.getVariable("isDecisionRejected") != null && kcontext.getVariable("isDecisionRejected").toString().equals("true")){
								if (indDealStage.get("stageValue").equals(
										"Complete")) {
									kcontext.setVariable("nextStatus",
											indDealStage.get("stageId"));
									break;
								}
						}
						else {
			...
			if (documentListArray != null) {
				documentListArray
						.forEach(item -> {
							...
							documentMap.put("expiryDate", documentEle
									.get("expiryDate") != null ? documentEle
